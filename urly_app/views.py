...
def index(request):
    ...
    return render_to_response('urly_app/index.htm', c)
...
def triage(request):
    ...
    if intent == 'preview':
        ...
        c['resource_title'] = tu
        c['delay_seconds'] = 7
        return render_to_response('urly_app/interstitial.htm', c)
    ...
    if intent == 'admin':
        return HttpResponseRedirect('http://go.spe.org/admin/urly_app/link/' + l.short_url_hash + '/')
...
def shorten(request, short_code_requested='', long_url_specified='', tracking_type_specified=''):
    if request.method == 'POST' and request.POST.get("canonical_url", ''):
        ...
        return render_to_response('urly_app/shorten.htm', c)
...
def get_link_info(request,short_code,recipients,short_base_path=''):
    ...
    c = m.count()
    if c == 0:
        _s = "[URLy Bird] Error: " + short_url + " does not exist"
        ...
        _s = "[URLy Bird] Error: " + short_url + " has multiple matches"
        _t = ""
        _h = ""
    else:
        l = m[0]
...
#            _r.append(re.sub(' .*$', '', l.pub_user))
        ...
        htmllines.append( "</dl>" )
        htmllines.append( '<hr />' )
        htmllines.append( '<div align="center"><small style="color:#ccc;"><a href="http://go.spe.org/shorten">URLy</a>, the URL shortener and campaign tracking tool</small></div>' )
        _h = "\n".join(htmllines)

    _f = '"URLy Bird" <noreply@spe.org>'
    ...
    if "@" in recipients:
        ...
        return HttpResponse("<h1>URLy</h1><h2>Link Details</h2>" + _h, content_type="text/html")
